...
async function getMoviesFromHTML(html) {
	...
	const movies = $('.amy-movie-item.entry-item')
		.map((i, el) => {
			...
			const country = back
				.find(
					'.amy-movie-field-language .amy-movie-custom-field-content'
				)
				.text()
				.trim();
			...
			return {
				...
				genre: back
					.find(
						'.amy-movie-field-amy_genre .amy-movie-custom-field-content a'
					)
					.map((i, genreEl) => $(genreEl).text())
					.get(),
				actors: back
					.find(
						'.amy-movie-field-amy_actor .amy-movie-custom-field-content a'
					)
